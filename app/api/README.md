...
| POST   | `/devices/{device_id}/acquire_max`          | Bearer | Own lock    | Capture full-memory-depth waveforms (MAX mode). Same query params as `/acquire`. Returns a `text/event-stream` SSE stream: `{"type":"progress", channel, channel_index, total_channels, batch, total_batches}` per batch; `{"type":"done", artifact_ids, acquisition_id, session_id, channels}` when complete; `{"type":"error", message}` on failure. |
