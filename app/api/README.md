...
| POST   | `/sessions/{session_id}/commit`                                   | Bearer | Upload all flagged artifacts as an `OSCILLOSCOPE` dataset. JSON body: `{experiment_id, lab_course?, exp_title?, group_name?, semester?, exp_description?, device_under_test?, measurement_purpose?, keywords?, data_quality?, external_parameters?, notes?}`. Derived properties (timestamps, channel count, has_screenshots, has_csv) are computed from the flagged artifacts. Returns `permId` + `artifact_count`. |
