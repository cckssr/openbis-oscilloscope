...
- `acquire_waveform_max(channel, progress_cb=None) -> WaveformData` — full-memory-depth acquisition. Default falls back to `acquire_waveform()` with a single synthetic progress event. Override in hardware drivers that support a batched read mode (see `RigolDS1000Driver`). `progress_cb(completed, total)` is called after each batch and is safe to call from a worker thread via `loop.call_soon_threadsafe`.
...
- `acquire_waveform_max(channel, progress_cb)` returns a 10× wider waveform and fires fake progress events for all simulated batches immediately (no delay), so the SSE progress bar works in `DEBUG=True` mode.
...
| `acquire_waveform_max(channel, progress_cb)` | MAX waveform mode. Reads total points from preamble, then fetches in batches of 250,000 using `waveform_start`/`waveform_stop`. Calls `progress_cb(batch, total)` after each batch. Does not stop/restart the scope (MAX mode reads from running acquisition).                                  |
