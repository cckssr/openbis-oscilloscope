...
| `config.py` | Pydantic `Settings` class. All environment variables are read here and nowhere else. Key settings: `REDIS_URL`, `OPENBIS_URL`, `OPENBIS_SPACE`, `BUFFER_DIR`, `OSCILLOSCOPES_CONFIG`, `LOCK_TTL_SECONDS`, `HEALTH_CHECK_INTERVAL_SECONDS`, `HEALTH_CHECK_TCP_TIMEOUT_SECONDS`, `HEALTH_CHECK_IDLE_TIMEOUT_SECONDS`, `TOKEN_CACHE_SECONDS`, `EOD_RESET_TIMEZONE`, `DEBUG`, `DEBUG_TOKEN`, `OPENBIS_BOT_USER`, `OPENBIS_BOT_PASSWORD`, `DRIVER_MAPPING_CONFIG`, `OPENBIS_EQUIPMENT_IP_FILTER`. |
