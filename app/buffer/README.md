...
| `create_commit_zip(session_id, flagged, extra_content?)`                          | `Path`               | Compress flagged artifacts into `commit_{session_id}.zip`. Channels sharing an `acquisition_id` are merged into a single multi-column CSV (`time_s, ch1_voltage_V, ch2_voltage_V, …`). Annotation text is used as the filename (slugified, deduplicated with `_2`, `_3` suffixes). Pass `extra_content` to inject additional text files (e.g. `{"dataset_metadata.json": "…"}`). |
