...
| GET    | `/devices/{device_id}/memory-depth`         | Bearer | —           | Current acquisition memory depth in samples. Returns `{device_id, memory_depth}`.                                                                                                                                                                                                                                  |
...
| POST   | `/sessions/{session_id}/commit`                                   | Bearer | Upload all flagged artifacts as an `OSCILLOSCOPE` dataset. JSON body: `{experiment_id, object_id?, lab_course?, exp_title?, group_name?, semester?, exp_description?, device_under_test?, measurement_purpose?, keywords?, data_quality?, external_parameters?, notes?}`. `experiment_id` must be the full collection path `/SPACE/PROJECT/COLLECTION`. Derived properties (timestamps, channel count, has_screenshots, has_csv) are computed from the flagged artifacts. Artifacts are compressed into a ZIP before upload; channels sharing an `acquisition_id` are merged into a single multi-column CSV; annotation text is used as the filename. When `OPENBIS_USE_DROPBOX=True` the ZIP (with an embedded `dataset_metadata.json`) is copied to `OPENBIS_DROPBOX_PATH` instead of being uploaded via pybis; `permId` will be `null` and `dropbox_file` contains the destination path. Returns `permId` + `artifact_count` (+ `dropbox_file` in dropbox mode). |
